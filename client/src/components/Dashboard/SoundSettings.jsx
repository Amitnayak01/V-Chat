  ...
  return (
    ...
    >
      ...
      {opt.id !== 'silent' && (
        ...
        >
          <span style={{ fontSize: 13 }}>{previewing ? '⏹' : '▶'}</span>
        </button>
      )}
    </motion.div>
  );
      ...
      >
        {uploaded ? (
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            <div style={{ width: 44, height: 44, borderRadius: 12, flexShrink: 0, background: T.greenL, border: `1px solid #BBF0D8`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 20 }}>🎵</div>
            <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 10, textAlign: 'center' }}>
            <div style={{ width: 48, height: 48, borderRadius: 14, background: dragging ? T.blueLH : T.border, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 22, transition: 'background .15s' }}>
              {dragging ? '🎯' : '📂'}
            </div>
            <div>
...
/* ── Ring Panel ── */
function RingPanel({ tabId, tones, accent, accentL, accentBd, tabSettings, tabCategory, update }) {
  const isMsg = tabId === 'message';
  const [previewing, setPreviewing] = useState(null);

  const selected = isMsg ? (tabSettings?.tone ?? tones[0].id) : (tabSettings?.ringtone ?? tones[0].id);
  const volume   = tabSettings?.volume   ?? 0.75;
  const vibrate  = tabSettings?.vibration ?? true;

  const setSelected = (id) => update(tabCategory, isMsg ? { tone: id } : { ringtone: id });
  const setVolume   = (v)  => update(tabCategory, { volume: v });
  const setVibrate  = (v)  => update(tabCategory, { vibration: v });
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 16px', background: T.surf2, border: `1px solid ${T.border}`, borderRadius: 12 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
...
export default function SoundSettings() {
  ...
  const tabSettings = activeTab === 'message' ? settings.messages
                    : activeTab === 'video'   ? settings.videoCall
                    : settings.audioCall;
  const tabCategory = activeTab === 'message' ? 'messages'
      ...
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        input[type=range]::-webkit-slider-thumb {
          ...
          cursor: pointer; border: 2px solid #fff; transition: transform .15s;
        }
        ...
        @media (max-width: 540px) {
          ...
          .ss-reset-btn { padding: 6px 8px !important; }
        }
      `}</style>

      <div className="ss-root" style={{
        minHeight: '100vh',
        background: `radial-gradient(ellipse 80% 50% at 50% -10%, ${tabMeta.accentL}, ${T.bg} 60%)`,
        transition: 'background .5s',
      }}>
        <div style={{ maxWidth: 680, margin: '0 auto', padding: 'clamp(24px,5vw,52px) clamp(14px,4vw,28px) 64px' }}>

          {/* ── Back button ── */}
          <button className="ss-back-btn" onClick={() => navigate('/dashboard/settings')}>
            ← Back to Settings
          </button>
          ...
          <motion.div initial={{ opacity: 0, y: -16 }} animate={{ opacity: 1, y: 0 }} transition={EASE} style={{ marginBottom: 28 }}>
            ...
            <p style={{ fontSize: 13.5, color: T.muted, marginTop: 6, lineHeight: 1.5 }}>
              Customize tones, volume and vibration for calls &amp; messages.
            </p>
          </motion.div>
          ...
          >
            ...
            <div style={{
              ...
              <p style={{ fontSize: 11.5, color: T.green, lineHeight: 1.4 }}>
