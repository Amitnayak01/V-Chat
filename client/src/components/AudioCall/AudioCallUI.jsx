 ...
 * v4.1: Video button wired — ends audio call, emits call-user, navigates to /room/:id
 */
...
const useActiveSpeaker = remoteStreams => {
  ...
  useEffect(()=>{
    ...
    remoteStreams.forEach((stream,uid)=>{
      ...
      try{const src=ctxRef.current.createMediaStreamSource(stream);const a=ctxRef.current.createAnalyser();a.fftSize=256;src.connect(a);arRef.current.set(uid,{a,d:new Uint8Array(a.frequencyBinCount)});}catch(_){}
    });
    for(const uid of arRef.current.keys()) if(!remoteStreams.has(uid)) arRef.current.delete(uid);
    const poll=()=>{
      let max=0,loudest=null;
      arRef.current.forEach(({a,d},uid)=>{a.getByteTimeDomainData(d);let sum=0;for(const v of d)sum+=(v-128)**2;const rms=Math.sqrt(sum/d.length);if(rms>max){max=rms;loudest=uid;}});
      setSpk(max>3?loudest:null);rafRef.current=requestAnimationFrame(poll);
    };
    poll();
...
const useNetworkQuality = remoteStreams => {
  ...
  useEffect(()=>{
    ...
    const id=setInterval(()=>{const c=navigator.connection||navigator.mozConnection||navigator.webkitConnection;if(c){const r=c.rtt??0;setQ(r<150?'good':r<400?'fair':'poor');}else setQ('good');},3000);
    setQ('good');
...
const WaveBars = ({active, color='#10b981', count=5, height=14}) => (
  <div style={{display:'flex',alignItems:'flex-end',gap:2,height,flexShrink:0}}>
    {Array.from({length:count},(_,i)=>{const hs=[.4,.75,.55,1,.65];return <span key={i} style={{width:2.5,height:active?`${hs[i%hs.length]*100}%`:'25%',background:color,borderRadius:3,transition:'height .1s',animation:active?`vm-wave .7s ease-in-out ${i*.1}s infinite alternate`:'none',opacity:active?1:.28}}/>;})}</div>
);
...
const Avatar = ({src, name, size=56, speaking=false, ringing=false}) => (
  <div style={{position:'relative',width:size,height:size,flexShrink:0}}>
    ...
    <div style={{position:'relative',width:'100%',height:'100%',borderRadius:'50%',overflow:'hidden',boxShadow:ringing?'0 0 0 3px rgba(16,185,129,.7), 0 0 0 6px rgba(16,185,129,.18), 0 8px 32px rgba(0,0,0,.55)':speaking?'0 0 0 2.5px #10b981, 0 0 18px rgba(16,185,129,.32), 0 6px 24px rgba(0,0,0,.5)':'0 4px 20px rgba(0,0,0,.55)',transition:'box-shadow .25s'}}>
      {src?<img src={src} alt={name} style={{width:'100%',height:'100%',objectFit:'cover'}}/>:<div style={{width:'100%',height:'100%',background:'linear-gradient(135deg,#0f766e,#0891b2)',display:'flex',alignItems:'center',justifyContent:'center',fontWeight:800,color:'#fff',fontSize:size*.38,userSelect:'none',letterSpacing:'-.02em'}}>{name?.[0]?.toUpperCase()??''}</div>}
    </div>
  </div>
);
...
/* Btn — v4.1: added disabled + title props */
    ...
    <div style={{position:'fixed',inset:0,zIndex:400,display:'flex',alignItems:'flex-end',justifyContent:'center'}} onClick={onClose}>
      ...
      <div onClick={e=>e.stopPropagation()} style={{position:'relative',zIndex:1,width:'100%',maxWidth:520,borderRadius:'24px 24px 0 0',background:'linear-gradient(180deg,#0d1b33,#081020)',border:'1px solid rgba(255,255,255,.1)',borderBottom:'none',boxShadow:'0 -20px 60px rgba(0,0,0,.8)',animation:'vm-drawer .28s cubic-bezier(.34,1.2,.64,1)',maxHeight:'82vh',overflow:'hidden',display:'flex',flexDirection:'column'}}>
        ...
        <div style={{overflowY:'auto',padding:'16px 20px 32px',flex:1}}>
          ...
          {devices.mics.length>0?devices.mics.map(d=><DeviceRow key={d.deviceId} device={d} selected={selectedMic===d.deviceId} onSelect={setSelectedMic} Icon={Mic}/>):<p style={{color:'rgba(255,255,255,.3)',fontSize:12,textAlign:'center',padding:'12px 0'}}>No microphones found</p>}
          ...
          {devices.speakers.length>0?devices.speakers.map(d=><DeviceRow key={d.deviceId} device={d} selected={selectedSpeaker===d.deviceId} onSelect={id=>{setSelectedSpeaker(id);onSpeakerChange(id);}} Icon={d.label?.toLowerCase().includes('headphone')||d.label?.toLowerCase().includes('earphone')?Headphones:Volume2}/>):<p style={{color:'rgba(255,255,255,.3)',fontSize:12,textAlign:'center',padding:'12px 0'}}>No output devices — browser may not support output selection</p>}
        </div>
      </div>
    </div>
  );
  ...
  <div style={{position:'fixed',inset:0,zIndex:400,display:'flex',alignItems:'center',justifyContent:'center',padding:20}} onClick={onClose}>
    ...
    <div onClick={e=>e.stopPropagation()} style={{position:'relative',zIndex:1,width:'100%',maxWidth:360,borderRadius:24,background:'linear-gradient(160deg,#0d1b33,#081020)',border:'1px solid rgba(255,255,255,.1)',boxShadow:'0 24px 64px rgba(0,0,0,.8)',padding:'24px 24px 28px',animation:'vm-cardin .25s cubic-bezier(.34,1.2,.64,1)'}}>
      <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:20}}>
        <div style={{display:'flex',alignItems:'center',gap:10}}>
          <div style={{width:38,height:38,borderRadius:12,background:'rgba(139,92,246,.2)',border:'1px solid rgba(139,92,246,.4)',display:'flex',alignItems:'center',justifyContent:'center'}}><UserPlus style={{width:18,height:18,color:'#c4b5fd'}}/></div>
          <div><h3 style={{color:'#fff',fontWeight:700,fontSize:15,margin:0}}>Add to Call</h3><p style={{color:'rgba(255,255,255,.4)',fontSize:11,margin:'2px 0 0'}}>Invite someone to join</p></div>
        </div>
        <button onClick={onClose} style={{width:30,height:30,borderRadius:'50%',background:'rgba(255,255,255,.08)',border:'1px solid rgba(255,255,255,.1)',display:'flex',alignItems:'center',justifyContent:'center',cursor:'pointer'}}><X style={{width:13,height:13,color:'#94a3b8'}}/></button>
      </div>
      ...
      <div style={{padding:'14px',borderRadius:14,background:'rgba(139,92,246,.08)',border:'1px solid rgba(139,92,246,.2)',textAlign:'center'}}>
        <p style={{color:'rgba(255,255,255,.5)',fontSize:12,margin:0,lineHeight:1.6}}>Conference call support requires a contact list integration.<br/><span style={{color:'#c4b5fd'}}>Connect your contacts provider to enable this.</span></p>
      </div>
    </div>
  </div>
);
...
══════════════════════════════════════════════════════════════════════════ */
const AudioCallUI = () => {
  const {callState,activeCall,incomingCall,remoteStreams,localStream,isMuted,callDuration,participants,callStatus,endCall,toggleMute,acceptCall,rejectCall} = useAudioCall();

  /* ── v4.1: video call hooks ─────────────────────────────────────────── */
  ...
  useEffect(()=>{
    ...
    if(callState==='idle'){setMounted(false);setMode('normal');setPos({x:0,y:0});stopRecording();setShowDeviceDrawer(false);setShowConferenceModal(false);setShowSecRow(false);}
  ...
  const startRecording=useCallback(()=>{
    ...
    try{
      ...
      rec.onstop=()=>{const url=URL.createObjectURL(new Blob(recChunks.current,{type:'audio/webm'}));Object.assign(document.createElement('a'),{href:url,download:`call-${Date.now()}.webm`}).click();URL.revokeObjectURL(url);};
  ...
  /* ── v4.1: handleSwitchToVideo ──────────────────────────────────────────
     1. endCall()        — cleans up audio peer connection + saves call history
     2. generateRoomId() — creates a fresh unique room ID
  ...
  /* Video btn disabled for group calls — no single peer to ring */
  ...
  const AudioEls=()=>Array.from(remoteStreams.entries()).map(([uid,s])=><RemoteAudio key={uid} userId={uid} stream={s} volume={volume} sinkId={activeSinkId}/>);
    ...
    <><GlobalStyles/><AudioEls/>
      <div style={{position:'fixed',zIndex:210,bottom:isMobile?76:24,left:isMobile?'50%':'auto',right:isMobile?'auto':24,transform:isMobile?'translateX(-50%)':'none',animation:'vm-cardin .22s ease'}}>
        <div style={{display:'flex',alignItems:'center',gap:10,padding:'9px 14px 9px 12px',borderRadius:999,background:'rgba(7,14,28,.97)',border:'1px solid rgba(255,255,255,.1)',boxShadow:'0 8px 32px rgba(0,0,0,.65)',backdropFilter:'blur(20px)',minWidth:220,maxWidth:isMobile?'calc(100vw - 48px)':300}}>
          ...
          <div style={{flex:1,minWidth:0}}><p style={{color:'#fff',fontWeight:700,fontSize:12,margin:0,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{peerName}</p><span style={{fontSize:10,fontFamily:'monospace',fontWeight:700,color:statusColor}}>{statusText}</span></div>
          ...
          <button onClick={()=>setMode('normal')} style={{width:32,height:32,borderRadius:'50%',background:'rgba(255,255,255,.07)',border:'1px solid rgba(255,255,255,.1)',display:'flex',alignItems:'center',justifyContent:'center',cursor:'pointer',flexShrink:0}}><ChevronUp style={{width:13,height:13,color:'#94a3b8'}}/></button>
        </div>
      </div>
    </>
  );
      ...
      <div style={{position:'fixed',inset:0,zIndex:200,display:'flex',flexDirection:'column',overflow:'hidden',background:'linear-gradient(160deg,#060e1f 0%,#0a1628 40%,#0d2137 70%,#060e1f 100%)',animation:'vm-fadein .18s ease'}}>
        ...
        <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',padding:'16px 20px 8px',position:'relative',zIndex:10,flexShrink:0}}>
          <div style={{display:'flex',alignItems:'center',gap:10}}>
            ...
            {noiseSuppression&&<div style={{display:'flex',alignItems:'center',gap:5,padding:'4px 10px',borderRadius:999,background:'rgba(20,184,166,.1)',border:'1px solid rgba(20,184,166,.3)'}}><span style={{width:6,height:6,borderRadius:'50%',background:'#14b8a6',animation:'vm-ns 2s ease infinite',flexShrink:0}}/><span style={{fontSize:10,fontWeight:700,color:'#2dd4bf'}}>Noise Filter</span></div>}
            <RecBadge rec={isRecording} dur={recDuration}/>
          </div>
          <div style={{display:'flex',gap:8}}>
            <button onClick={()=>setMode('minimized')} style={{width:38,height:38,borderRadius:'50%',background:'rgba(255,255,255,.07)',border:'1px solid rgba(255,255,255,.09)',display:'flex',alignItems:'center',justifyContent:'center',cursor:'pointer'}} title="Minimize"><ChevronDown style={{width:15,height:15,color:'#94a3b8'}}/></button>
            <button onClick={()=>setMode('normal')} style={{width:38,height:38,borderRadius:'50%',background:'rgba(255,255,255,.07)',border:'1px solid rgba(255,255,255,.09)',display:'flex',alignItems:'center',justifyContent:'center',cursor:'pointer'}} title="Windowed"><Minimize2 style={{width:15,height:15,color:'#94a3b8'}}/></button>
          </div>
        </div>
        <div style={{flex:1,display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',gap:20,padding:'0 24px',position:'relative',zIndex:10,overflow:'hidden'}}>
          {!isGroup?(<>
            ...
            <div style={{textAlign:'center'}}>
              ...
              {isOffline&&<div style={{display:'inline-flex',alignItems:'center',gap:6,marginTop:12,padding:'5px 14px',borderRadius:999,background:'rgba(251,191,36,.1)',border:'1px solid rgba(251,191,36,.3)'}}><span style={{width:6,height:6,borderRadius:'50%',background:'#fbbf24',flexShrink:0,animation:'vm-rec 1.5s ease infinite'}}/><span style={{fontSize:11,fontWeight:700,color:'#fbbf24'}}>User offline — ringing when they connect</span></div>}
            </div>
          </>):(<>
            <div style={{textAlign:'center'}}>
              <div style={{display:'flex',alignItems:'center',justifyContent:'center',gap:8,marginBottom:6}}><Users style={{width:18,height:18,color:'#10b981'}}/><h2 style={{color:'#fff',fontWeight:700,fontSize:20,margin:0}}>{peerName}</h2></div>
            ...
            <div style={{display:'flex',flexWrap:'wrap',justifyContent:'center',gap:4,maxWidth:480}}>
              {participants.length>0?participants.map(p=><ParticipantTile key={p.userId} participant={p} speaking={activeSpeaker===p.userId} large/>):<p style={{color:'#475569',fontSize:14}}>Waiting for others to join…</p>}
            </div>
          </>)}
        </div>
  ...
  /* ── NORMAL CARD ─────────────────────────────────────────────────────── */
  const cardPos=isMobile?{position:'fixed',left:0,right:0,bottom:0,zIndex:200}:{position:'fixed',right:Math.max(24,24-pos.x),bottom:Math.max(24,24-pos.y),zIndex:200,width:360};
          ...
          {!isMobile&&(
            <div onMouseDown={onDragStart} onTouchStart={onDragStart} style={{display:'flex',alignItems:'center',justifyContent:'space-between',padding:'10px 14px 2px',cursor:'grab',userSelect:'none'}}>
              <div style={{display:'flex',alignItems:'center',gap:8}}>
                ...
                <RecBadge rec={isRecording} dur={recDuration}/>
                {isConnected&&networkQuality!=='unknown'&&<div style={{display:'flex',alignItems:'center',gap:4,padding:'2px 7px',borderRadius:999,background:'rgba(255,255,255,.05)',border:'1px solid rgba(255,255,255,.08)'}}>{networkQuality==='poor'?<WifiOff style={{width:9,height:9,color:nqColor}}/>:<Wifi style={{width:9,height:9,color:nqColor}}/>}<span style={{fontSize:9,fontWeight:700,color:nqColor}}>{networkQuality==='good'?'HD':networkQuality==='fair'?'SD':'Weak'}</span></div>}
              </div>
              <div style={{display:'flex',alignItems:'center',gap:2}}>
                {!isIncoming&&<><button onClick={()=>setMode('minimized')} onMouseDown={e=>e.stopPropagation()} style={{width:22,height:22,borderRadius:'50%',background:'none',border:'none',display:'flex',alignItems:'center',justifyContent:'center',cursor:'pointer'}} title="Minimize"><ChevronDown style={{width:12,height:12,color:'#475569'}}/></button><button onClick={()=>setMode('fullscreen')} onMouseDown={e=>e.stopPropagation()} style={{width:22,height:22,borderRadius:'50%',background:'none',border:'none',display:'flex',alignItems:'center',justifyContent:'center',cursor:'pointer'}} title="Fullscreen"><Maximize2 style={{width:11,height:11,color:'#475569'}}/></button></>}
          ...
          <div style={{padding:isMobile?'16px 22px 6px':'12px 20px 6px'}}>
            <div style={{display:'flex',flexDirection:'column',alignItems:'center',gap:12,marginBottom:isIncoming?20:14}}>
              ...
              <div style={{textAlign:'center',width:'100%'}}>
                {isIncoming&&<div style={{display:'flex',alignItems:'center',justifyContent:'center',gap:8,marginBottom:7}}><WaveBars active color="#10b981" count={5} height={12}/><span style={{fontSize:10,fontWeight:700,letterSpacing:'.14em',textTransform:'uppercase',color:'#10b981'}}>Incoming audio call</span><WaveBars active color="#10b981" count={5} height={12}/></div>}
                <h2 style={{color:'#fff',fontWeight:800,fontSize:isMobile?(isIncoming?21:18):(isIncoming?19:16),margin:'0 0 6px',letterSpacing:'-.025em',display:'flex',alignItems:'center',justifyContent:'center',gap:7}}>
                  {isGroup&&<Users style={{width:15,height:15,color:'#10b981',flexShrink:0}}/>}{peerName}
                </h2>
                ...
                {isOffline&&<div style={{display:'inline-flex',alignItems:'center',gap:5,marginTop:8,padding:'3px 10px',borderRadius:999,background:'rgba(251,191,36,.1)',border:'1px solid rgba(251,191,36,.3)'}}><span style={{width:5,height:5,borderRadius:'50%',background:'#fbbf24',animation:'vm-rec 1.5s ease infinite'}}/><span style={{fontSize:10,fontWeight:700,color:'#fbbf24'}}>Ringing when they reconnect</span></div>}
